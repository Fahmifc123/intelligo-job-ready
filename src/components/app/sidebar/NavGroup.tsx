...
export function NavGroup({title, items, className}: NavGroup) {
  ...
  return (
    <SidebarGroup>
      <SidebarGroupLabel className={cn("text-sidebar-foreground/70", className)}>
        {title}
      </SidebarGroupLabel>
      <SidebarMenu>
        {items.map((item) => {
          ...
          return <SidebarMenuCollapsible key={key} item={item} href={href} className={className ?? ""}/>
        })}
      </SidebarMenu>
    </SidebarGroup>
  )
    ...
    <SidebarMenuItem>
      <SidebarMenuButton
        ...
        className={cn(
          ...
          status && "bg-gradient-to-r from-sidebar-primary to-sidebar-primary/80 text-sidebar-primary-foreground"
        )}
      >
